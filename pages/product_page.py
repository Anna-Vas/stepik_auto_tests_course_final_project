...
class ProductPage(BasePage):
    ...
    def should_be_correct_product_name(self):
        ...
        message = self.browser.find_element(*ProductPageLocators.MESSAGE_ADDED_TO_CART).text
        message_product_name = " ".join(message.split()[:-6])
    ...
    def should_be_correct_cart_price(self):
        ...
        message_cart_total_price = message_cart_total.split()[-1]
        assert product_price == message_cart_total_price,\
            f"Incorrect product price in the message: price should be {product_price}, not {message_cart_total_price}"
